...
enum AssignPhotoAction {
    case InitialData(photos: [PHAsset], epochDay: Int?, type: PhotoType)
...
enum AssignPhotoResult {
    ...
    case Display(asset: PHAsset, date: Date, photoDate: Date?, type: PhotoType, index: Int, count: Int)
    case SavingImage(asset: PHAsset, date: Date, photoDate: Date?, type: PhotoType, index: Int, count: Int)
}
...
class AssignPhotoDomain {
    ...
    func assignPhotoActionsToResults(_ dataController: DataController) -> ObservableTransformer<AssignPhotoAction, AssignPhotoResult> {
        func getObservableFromAction(action: AssignPhotoAction) -> Observable<AssignPhotoResult> {
            switch(action){
            case .InitialData(let photos, let epochDay, let type):
                self.photos = photos
                self.type = type
            ...
            case .ShowDateDialog(let index):
                ...
                return Observable.just(.Display(asset: photos[index], date: date, photoDate: photoDate, type: type, index: index, count: photos.count))
            ...
            case .Save(let index):
                ...
                DispatchQueue.global(qos: .userInitiated).async {
                    requestImageToSave(asset: asset, failure: {
                ...
                return Observable.just(AssignPhotoResult.SavingImage(asset: asset, date: date, photoDate: photoDate, type: type, index: index, count: photos.count))
        ...
        func loadImage(_ index: Int) -> AssignPhotoResult {
            ...
            return .Display(asset: asset, date: date, photoDate: photoDate, type: type, index: index, count: photos.count)
        }
        
        func moveToNext(_ index: Int) -> AssignPhotoResult {
            if index + 1 >= self.photos.count {
                self.viewEffectRelay.accept(.CompletedAssigning)
                return AssignPhotoResult.Loading(index: index, count: self.photos.count)
            } else {
                return AssignPhotoResult.Display(asset: self.photos[index + 1], date: self.date, photoDate: self.photoDate, type: self.type, index: index + 1, count: self.photos.count)
            }
        }
